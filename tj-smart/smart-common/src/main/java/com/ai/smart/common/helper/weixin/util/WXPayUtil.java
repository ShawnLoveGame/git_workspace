package com.ai.smart.common.helper.weixin.util;



import com.ai.smart.common.helper.util.*;
import com.ai.smart.common.helper.weixin.entity.Location;
import com.ai.smart.common.helper.weixin.pay.entity.notify.NotifyReceive;
import com.ai.smart.common.helper.weixin.pay.entity.notify.NotifyReply;
import com.ai.smart.common.helper.weixin.pay.entity.req.*;
import com.ai.smart.common.helper.weixin.pay.entity.resp.*;
...
public class WXPayUtil {
	 ...
	 * @throws Exception
	 */
	public  String weixin_pay(String order_price,String body,String out_trade_no,String notify_url) throws Exception {
		String path = this.getClass().getResource("/").getPath();
		String websiteURL = (path.replace("/build/classes", "").replace("%20"," ").replace("classes/", "") + "classes/temp.properties");
//		String websiteURL = (path.replace("/build/classes", "").replace("%20"," ").replace("classes/", "") + "classes/temp.properties").replaceFirst("/", "");
		// 账号信息
		String appid = PropertiesUtil.getValue(websiteURL, "app_id");  // appid
		String mch_id = PropertiesUtil.getValue(websiteURL, "merchant_id"); // 商业号
		String key = PropertiesUtil.getValue(websiteURL, "api_key"); // key
...
//		String body = "testHotel";   // 商品名称
//		String out_trade_no = "11338"; // 订单号

		String spbill_create_ip = PropertiesUtil.getValue(websiteURL, "current_server_ip");
		...
		if (packageParams.size() > 0) {
			...
			for (SortedMap.Entry<Object,Object> param: packageParams.entrySet()) {
				infoStr = infoStr + " " + param.getKey() + " : " + param.getValue();
			}

			logger.info("请求参数： "+infoStr);
		} else {
			logger.error("参数获取失败");
		}
		...
		logger.info("请求参数转换的xml内容："+requestXML);

		String resXml = HttpUtil.postData(PropertiesUtil.getValue(websiteURL, "ufdoder_url"), requestXML);

		logger.info("请求成功后的内容："+resXml);
...
"</xml>";
	...
	public    PayOrderPay weixin_pay(String order_price, String body, String out_trade_no, String app, String notify_url,String UserIp) throws Exception {
        
		String path = this.getClass().getResource("/").getPath();
		String websiteURL = (path.replace("/build/classes", "").replace("%20"," ").replace("classes/", "") + "classes/weichatApp.properties");
//		String websiteURL = (path.replace("/build/classes", "").replace("%20"," ").replace("classes/", "") + "classes/temp.properties").replaceFirst("/", "");
		// 账号信息
		String appid = PropertiesUtil.getValue(websiteURL, "app_id");  // appid
		String mch_id = PropertiesUtil.getValue(websiteURL, "merchant_id"); // 商业号
		...
		String nonce_str = strTime + strRandom;
		//Subject currentUser = SecurityUtils.getSubject();
	    //String attach = (String) currentUser.getSession().getAttribute("userId");
	   // System.out.println("==============attach"+attach);
//	    JSONObject json1=new JSONObject();
//	    JSONObject json2=new JSONObject();
//	    json1.put("package_name", "io.dcloud.H581667D1");
//	    json1.put("app_name", "党建");
//	    json1.put("type", "Android");
//        json2.put("h5_info", json1);
//        String scene_info=json2.toString();
//		String order_price = "1"; // 价格   注意：价格的单位是分
//		String body = "testHotel";   // 商品名称
//		String out_trade_no = "11338"; // 订单号
...
//		packageParams.put("scene_info", scene_info);
        ...
        String signAgain = PayCommonUtil.createSign("UTF-8", signParam,key);		
        signParam.put("sign", signAgain);
        payOrderPay.setAppid(appid);
        payOrderPay.setPrepayid(prepay_id);
        payOrderPay.setPartnerid(mch_id);
        payOrderPay.setPackageEx("Sign=WXPay");
        payOrderPay.setNoncestr(nonce_str);
        payOrderPay.setSign(signAgain);
//      String timeStamp = Long.toString(new Date().getTime() / 1000);
        payOrderPay.setTimestamp(seconds);
        payOrderPay.setOutodernum(out_trade_no);
		}
		return payOrderPay;	}
}